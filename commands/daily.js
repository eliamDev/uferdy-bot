...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
            subcommand
                .setName('set')
                .setDescription('Establecer bonificaciones diarias (solo desarrollador)')
                .addStringOption(option =>
                    option.setName('categoria1')
...
async function handleView(interaction) {
    ...
    if (bonusData.bonuses.length === 0) {
        embed.setDescription('❌ No hay bonificaciones configuradas para hoy.')
            .addFields({
                name: '🔍 Cómo verificar en el juego',
                value: 'Presiona **Alt+Y** o ve a **Actividades > Bonificaciones de Producción**',
                inline: false
            });
    } else {
        embed.setDescription('✅ Bonificaciones activas para hoy:')
            .addFields(
        ...
        if (bonusData.lastUpdate) {
            embed.setFooter({ text: `Última actualización: ${new Date(bonusData.lastUpdate).toLocaleString('es-ES')}` });
        }
    }
...
async function handleSet(interaction) {
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields(
            { name: 'Bonificación 1', value: `${CATEGORIES[categoria1]} - **${bonus1}%**`, inline: true },
            { name: 'Bonificación 2', value: `${CATEGORIES[categoria2]} - **${bonus2}%**`, inline: true }
        )
        .setTimestamp();
